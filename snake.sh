...
echo""
echo "	SSSSSSS  NN      NN        A        KK   KK   EEEEEEE"
echo "	SS       NNNN    NN       AAA       KK  KK    EE"
echo "	SS       NN NN   NN      AA AA      KK KK     EE"
echo "	SSSSSSS  NN  NN  NN     AA   AA     KK        EEEEEE"
echo "	     SS  NN   NN NN    AAAAAAAAA    KK KK     EE"
echo "	     SS  NN     NNN   AA       AA   KK  KK    EE"
echo "	SSSSSSS  NN      NN  AA         AA  KK   KK   EEEEEEE"
echo ""
...
ordenarScore() {
	...
	for ((i = 0; i<${#arrName[*]}; i++))
	do		   
	 for((j = 0; j<${#arrName[*]}-i-1; j++))
	 do		   
	  if [[ ${arrScore[j]} -lt ${arrScore[$((j+1))]} ]]
	  then
		tempScore=${arrScore[j]}
		tempName=${arrName[j]}
	...
	echo "${arrName[0]},${arrScore[0]}" > HighScores.txt
	for((ind = 1; ind<5; ind++))
	 do
		echo "${arrName[$ind]},${arrScore[$ind]}" >> HighScores.txt
	done
...
highScores() {
	...
	echo "$nickname,$score" >> HighScores.txt
...
game_loop() {
    ...
    highScores $1 $score
    echo -e "${text_color}Game Over, $1 $no_color"
...
while [ $option -ne 4 ]; do
	...
	case $option in
			1)
			   ...
			   read nickname
			   echo "Buena suerte, $nickname"
			   echo "H: Izquierda, J: Abajo, K: Arriba, L Derecha (Presiona enter para continuar)"
			   read
				init_game
				init_snake
				give_food
				...
				;;
			2)
				echo "Mejores puntuaciones"
				count=1
				while IFS="," read -r name score
				do
					echo "$count ___ $name: $score"
					count=$(( $count + 1 ))
				done < HighScores.txt
				;;
			3)
